...
public class SelectProSpecWindow extends PopupWindow {
    ...
    private int type = 1;//(1-普通商品 2-限时购)
    private final static int TYPE_ADD = 1;
    private final static int TYPE_BUY_NOW = 2;


    public SelectProSpecWindow(Context context, final ProductDetail productDetail, int type) {
        ...
        call.enqueue(new Callback<ResultDO<ProductSpec>>() {
            ...
            public void onResponse(Call<ResultDO<ProductSpec>> call, Response<ResultDO<ProductSpec>> response) {
                ...
                if (resultDO.getCode() == 0) {
                    productPrice.setText(type == 2 ? String.format("¥%s", resultDO.getResult().getLimitPrice()) : String.format("¥%s", resultDO.getResult().getPrice()));
                    productStoreNum.setText(type == 2 ? String.format("库存:%s", resultDO.getResult().getLimitValue()) : String.format("库存:%s", resultDO.getResult().getValue()));
        ...
        switch (v.getId()) {
            ...
            case R.id.buy_now:
               ...
               /* if (buyNum > storeNum) {
                ...
                if (onSelectListener != null && specId != 0) {
                    onSelectListener.onSelect(specId, buyNum, TYPE_BUY_NOW);
                }
                dismiss();
                break;
            case R.id.btnCancel:
    ...
    private void applyAddProduct() {
        ((ProductDetailsActivity) (mContext)).showLoadingDialog(0);
        ...
        call.enqueue(new Callback<ResultDO>() {
            ...
            public void onResponse(Call<ResultDO> call, Response<ResultDO> response) {
                ((ProductDetailsActivity) (mContext)).dissMissDialog();
    ...
    public interface OnSelectListener {
        void onSelect(long paramId, int number, int buyType);
    }
