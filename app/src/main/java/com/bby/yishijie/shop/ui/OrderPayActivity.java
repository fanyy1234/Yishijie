...
import com.bby.yishijie.shop.entity.GroupBuyOrder;
...
public class OrderPayActivity extends BaseActivity {
    ...
    private void initView() {
        ...
        groupBuyOrder = (GroupBuyOrder) getIntent().getSerializableExtra("groupOrder");
        ...
        totalMoney.setText(String.format("¥%s", order.getRealPay().setScale(2, RoundingMode.HALF_UP)));
        integralMoney.setText(String.format("¥%s", order.getScoreFee().setScale(2, RoundingMode.HALF_UP)));
        balanceMoney.setText(String.format("¥%s", order.getMoneyFee().setScale(2, RoundingMode.HALF_UP)));
        money = String.format("%s", order.getRealPay().setScale(2, RoundingMode.HALF_UP));
    ...
    private void getPayInfo() {
        Call<ResultDO<PayInfo>> call = null;
//        if (isGroupBuy) {
//            call = ApiClient.getApiAdapter().getGroupPayInfo(orderId);
//            BaseApp.getInstance().setPayType(Constant.TYPE_GROUP);
//            BaseApp.getInstance().setGroupBuyOrder(groupBuyOrder);
//        } else {
            call = ApiClient.getApiAdapter().getNormalPayInfo2(orderId);
            BaseApp.getInstance().setPayType(Constant.TYPE_NORMAL);
            BaseApp.getInstance().setWxPayFlag(2);
            BaseApp.getInstance().setShopOrder(order);
//        }
        showLoadingDialog(0);
            ...
            switch (msg.what) {
                case SDK_PAY_FLAG: {
                    ...
                    if (TextUtils.equals(resultStatus, "9000")) {
                        ...
                        Intent intent = null;
//                        if (isGroupBuy) {
//                            intent = new Intent(mContext, GroupBuyOrderListActivity.class);
//                        } else {
