...
import com.bby.yishijie.shop.adapter.OrdetItemListAdapter;
    ...
    private void initView() {
        ...
        if (order.getIsBuy() == 2) {
            ...
            switch (order.getStatus()) {
                ...
                case 1:
                    orderStatus.setText("待发货");
                    if (order.getRealPay().compareTo(BigDecimal.ZERO) == 0 &&
                            order.getMoneyFee().compareTo(BigDecimal.ZERO) == 0) {
                        btn_layout.setVisibility(View.GONE);
                    } else {
...
//                type = 1;
//            }
//            OrderProductAdapter adapter = new OrderProductAdapter(mContext, order.getList(), order.getIsBuy() == 1 ? 0 : 1,type,
//                    order.getScoreFee().compareTo(BigDecimal.ZERO) > 0);
...
//        }
    ...
    private void setList() {
        ...
        isUsedScore = order.getScoreFee().compareTo(BigDecimal.ZERO) > 0;

        if (!order.getDatas().isEmpty()) {
            OrdetItemListAdapter adapter = new OrdetItemListAdapter(mContext, order.getDatas(), isUsedVoucher, isUsedScore, order.getIsBuy() == 1 ? 0 : 1, order);
            listView.setAdapter(adapter);
            adapter.notifyDataSetChanged();
        }
    }
    ...
    private void getOrderPay() {
        Call<ResultDO<Order>> call = ApiClient.getApiAdapter().getOrderPay2(order.getId());
