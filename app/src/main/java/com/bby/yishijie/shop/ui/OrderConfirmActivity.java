...
//import com.bby.yishijie.member.ui.mine.SettingPwdActivity;
...
//import com.bby.yishijie.shop.entity.PayInfo;
...
//    LinearLayout selectIntegral;
//    @Bind(R.id.integral_not_enough)
//    TextView intergralNotEnough;
...
//    private String message, balanceMoney, realName, idNo;
//    private int payType = 1;
//    private IWXAPI api;
//    private BigDecimal bd_Integral = BigDecimal.ZERO;
//    private BigDecimal bd_Balance = BigDecimal.ZERO;
//    private int payScore;//
//    private BigDecimal needPayMoney = BigDecimal.ZERO;//结算变化最终需要的
//    private BigDecimal integralMoney = BigDecimal.ZERO;//积分商品的钱
...
//    }
//
//    private void reCountCouponMoney() {
//        //清空优惠券
//        couponNum.setText("未使用");
//        couponMoney = 0;
//        voucherId = null;
//        needPayMoney = cartPay.getAmount().subtract(integralMoney);
//        reCountTotalMoney();
//    }
//
//
...
//        if (cartPay != null) {
//            totalMoney.setText(String.format("¥%s", cartPay.getAmount().setScale(2, RoundingMode.HALF_UP)));
...
////                OrderConfirmProAdapter adapter = new OrderConfirmProAdapter(mContext, cartPay.getList());
////                listView.setAdapter(adapter);
////                adapter.notifyDataSetChanged();
////            }
//
//            if (cartPay.getTotalScore() > cartPay.getMyScore()) {
//                integralCheckbox.setVisibility(View.GONE);
//                integralCheckbox.setChecked(false);
//                intergralNotEnough.setVisibility(View.VISIBLE);
//                bd_Integral = BigDecimal.ZERO;
//            } else {
//                integralCheckbox.setVisibility(View.VISIBLE);
//                intergralNotEnough.setVisibility(View.GONE);
//                payScore = cartPay.getTotalScore();
//                // integralMoney=cartPay.getScoreProductFee();
//                // bd_Integral = BigDecimal.valueOf(cartPay.getTotalScore());
//                bd_Integral = BigDecimal.ZERO;
//                integralMoney = BigDecimal.ZERO;
//            }
//            selectIntegral.setVisibility(cartPay.getTotalScore() == 0 ? View.GONE : View.VISIBLE);
//            //bd_Balance = cartPay.getAmount().subtract(integralMoney).add(cartPay.getPostFee());
//            bd_Balance = BigDecimal.ZERO;
//            needPayMoney = cartPay.getAmount().subtract(integralMoney);
//            integralNum.setText(cartPay.getMyScore() + "积分" + "(需" + cartPay.getTotalScore() + "积分)");
//            balance.setText(String.format("余额%s", cartPay.getMyMoney().setScale(2, RoundingMode.HALF_UP)));
//            setBottomText();
//            llId.setVisibility(isCross == 1 ? View.VISIBLE : View.GONE);
//            llRealName.setVisibility(isCross == 1 ? View.VISIBLE : View.GONE);
//            if (cartPay.getIsOut() == 1 || isCross == 1) {
//                sendType = 1;
//                selectSendMethod.setText("邮寄");
//            }
//        }
//
//        integralCheckbox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
//            @Override
//            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
//                if (isChecked) {
//                    bd_Integral = BigDecimal.valueOf(cartPay.getTotalScore());
...
//                        integralCheckbox.setEnabled(false);
//                    } else {
//                        integralCheckbox.setEnabled(true);
//                    }
//                    bd_Integral = BigDecimal.ZERO;
//                    integralMoney = BigDecimal.ZERO;
//                    reCountCouponMoney();
...
//        if (bd_Balance.compareTo(cartPay.getMyMoney()) < 1 && bd_Balance.compareTo(BigDecimal.ZERO) > 0) {
//            totalMoneyBottom.append(SpannalbeStringUtils.setTextColor("¥" + cartPay.getAmount().add(postFee).
//                            subtract(integralMoney).subtract(bd_Balance).setScale(2, RoundingMode.HALF_UP),
//                    getResources().getColor(R.color.colorPrimary)));
//        } else if (bd_Balance.compareTo(cartPay.getMyMoney()) >= 1 && bd_Balance.compareTo(BigDecimal.ZERO) > 0) {
//            bd_Balance = cartPay.getMyMoney();
//            totalMoneyBottom.append(SpannalbeStringUtils.setTextColor("¥" + cartPay.getAmount().add(postFee).
//                            subtract(integralMoney).subtract(bd_Balance).setScale(2, RoundingMode.HALF_UP),
//                    getResources().getColor(R.color.colorPrimary)));
//        } else {
//            totalMoneyBottom.append(SpannalbeStringUtils.setTextColor("¥" + cartPay.getAmount().add(postFee).
//                            subtract(integralMoney).setScale(2, RoundingMode.HALF_UP),
//                    getResources().getColor(R.color.colorPrimary)));
//
//
//        }
...
//        showLoadingDialog(0);
//        Call<ResultDO<com.bby.yishijie.member.entity.Address>> call = ApiClient.getApiAdapter().getDefaultAddr(memberId);
//        call.enqueue(new Callback<ResultDO<com.bby.yishijie.member.entity.Address>>() {
//            @Override
//            public void onResponse(Call<ResultDO<com.bby.yishijie.member.entity.Address>> call, Response<ResultDO<com.bby.yishijie.member.entity.Address>> response) {
//                if (isFinish) {
//                    return;
//                }
//                dissMissDialog();
//                ResultDO<com.bby.yishijie.member.entity.Address> resultDO = response.body();
//                if (resultDO == null) {
//                    return;
//                }
...
//
//            @Override
//            public void onFailure(Call<ResultDO<com.bby.yishijie.member.entity.Address>> call, Throwable t) {
//                dissMissDialog();
...
//                if (needPayMoney.compareTo(coupMoney) > -1) {
//                    reCountTotalMoney();
//                } else {
//                    voucherId = null;
//                    couponMoney = 0;
//                    ToastUtils.showToast(mContext, "请选择正确的优惠券");
//                    return;
//                }
//
//                break;
//            case REQUEST_PWD:
//                cartPay.setPayPassword(1);
//                showPwdDialog();
//                break;
//
//        }
...
//        }
//        if (balanceCheckbox.isChecked()) {
//            bd_Balance = cartPay.getAmount().subtract(integralMoney).add(postFee).subtract(coupMoney);
//        } else {
//            bd_Balance = BigDecimal.ZERO;
//        }
//        setBottomText();
...
//        score = bd_Integral.intValue();
//        message = editRemark.getText().toString().trim();
//        if (score > 0 || bd_Balance.compareTo(BigDecimal.ZERO) > 0) {
//            balanceMoney = String.valueOf(bd_Balance);
//            //余额支付
//            if (cartPay.getPayPassword() == 1) {
//                //有支付密码 密码验证
//                showPwdDialog();
//            } else {
//                showSettingDialog();
...
//        Call<ResultDO<Order>> call = null;
//        showLoadingDialog(0);
//        if (!TextUtils.isEmpty(cartIds)) {
//            //购物车提交订单
//            call = ApiClient.getApiAdapter().createOrderNew(address.getId(), cartIds, voucherId, score, balanceMoney, message, 2, idNo, realName,
//                    isActive == 0 ? null : 1, sendType);
...
//                        if (resultDO.getResult().getRealPay().compareTo(BigDecimal.ZERO) > 0) {
//                            if (payType == 1) {
//                                getPayInfo();
//                            } else if (payType == 2) {
//                                aliPay();
...
//                intent.putExtra("fromOrderPay", true);
...
//                payPwd = edtPassword.getText().toString();
...
//        Call<ResultDO<Integer>> call = ApiClient.getApiAdapter().checkPayPwd(memberId, MD5Utils.MD5(payPwd));
...
//        showLoadingDialog(0);
//        Call<ResultDO<Order>> call = ApiClient.getApiAdapter().createOrderBuyNow(address.getId(), cartId, voucherId, score, balanceMoney, message, 2, idNo,
//                realName, sendType);
//        call.enqueue(new Callback<ResultDO<Order>>() {
//            @Override
//            public void onResponse(Call<ResultDO<Order>> call, Response<ResultDO<Order>> response) {
...
//        Call<ResultDO<PayInfo>> call = ApiClient.getApiAdapter().getNormalPayInfo2(order.getId());
